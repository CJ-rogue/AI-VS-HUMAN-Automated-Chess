    ...
    switch (sequence) {
        ...
        case end:
            if (game_active && (current_time - last_end_press_time > debounce_delay)) {
                ...
                Serial.println("END button pressed. Stopping game...");
                stopGame();
            } else if (!game_active && (current_time - last_end_press_time > debounce_delay)) {
                last_end_press_time = current_time;
                Serial.println("No game in progress. Press START to begin.");
                sendTFTMessage("\n\n       No game\n\n     in progress");
                delay(1500);
                lcdDisplay();
            }
        ...
        case move_confirm:
            if (game_active && !difficulty_selected) {  
                Serial.println("Please select difficulty");
                sendTFTMessage("\n    Please select\n"
                              "\n     difficulty\n\n"
                              "\n    EASY or HARD");
                delay(1500);
            ...
            if (game_active) {
                char move[4];
                Serial.println("Enter move (e.g., e2e4:0 or e4d5:1):");
                sendTFTMessage("\n\n    AI Thinking...");
                delay(1000);
                sendTFTMessage("\n\n     AI Moving...");
                parseUserInput(move, capture_flag);
                piece_movement(move, capture_flag);
                sendTFTMessage("     Please make\n\n"
                               "      your move"
                               "\n\n\n\n  Press Move Button");
            } else {
                Serial.println("No game in progress");
                sendTFTMessage("\n\n       No game\n\n     in progress");
                delay(1500);
                lcdDisplay();
            }
...
//************************************  START GAME
void startGame() {
  if (!game_active) {  // Only start if not already active
    game_active = true;
    ...
    Serial.println("Select difficulty (Easy or Hard) to continue.");
    sendTFTMessage("\n    Please select\n"
                   "\n     difficulty\n\n"
                   "\n    EASY or HARD");
  } else {
    Serial.println("Game already in progress. START button ignored.");
  }
}
...
void stopGame() {
    if (game_active) {    
        ...
        trolley_coordinate_X = 5;  // Reset to initial position
        trolley_coordinate_Y = 7;

        Serial.println("Game stopped. Press START to restart.");
        sendTFTMessage("\n  Stopping game...");
        delay(1500);
        lcdDisplay();
    } else {
        Serial.println("Game is already stopped. END button ignored.");
    }
}
...
bool checkStopCondition() {
    ...
    if (digitalRead(END_BUTTON) == LOW) {  
        ...
        Serial.println("Game interrupted. Stopping...");
        stopGame();
...
void calibratePosition() {
    ...
    Serial.println("Moving towards X-axis limit switch...");
    while (digitalRead(LIMIT_X) == HIGH) {
        if (stop_execution) return;
        motor(B_T, SPEED_SLOW, calibrate_speed);
    }
    if (stop_execution) return;
    Serial.println("X-axis limit switch reached!");

    // Move to Y limit (bottom side)
    Serial.println("Moving towards Y-axis limit switch...");
    while (digitalRead(LIMIT_Y) == HIGH) {
        if (stop_execution) return;
        motor(L_R, SPEED_SLOW, calibrate_speed);
    }
    if (stop_execution) return;
    Serial.println("Y-axis limit switch reached!");

    delay(500);
    if (stop_execution) return;

    // Move to starting position (e7)
    Serial.print("Moving to starting position: X=");
...
// ******************************* USER INPUT
void parseUserInput(char *move, byte &capture_flag) {
    String input;
    ...
    while (true) {
        ...
        if (input.length() == 6 && input[4] == ':') {
            break; // Valid input, exit loop
        }

        Serial.println("Invalid move! Use format e2e4:1 or e4d5:0. Try again.");
    }

    // Extract move details
    move[0] = tolower(input[0]);  
    move[1] = input[1];           
    move[2] = tolower(input[2]);  
    move[3] = input[3];           
    capture_flag = input[5] - '0';  

    Serial.print("Parsed Move: ");
    Serial.print(move[0]); Serial.print(move[1]);
    Serial.print(" to ");
    Serial.print(move[2]); Serial.print(move[3]);
    Serial.print(" | Capture: ");
    Serial.println(capture_flag == 0 ? "Yes" : "No");
}
...
void piece_movement(char *move, byte capture_flag) {
  ...
  for (byte i = capture_flag; i < 2; i++) {
    ...
    if (i == 0) { // 0 means capture, trolley will go to arrival
      ...
      if (stop_execution) return;
      Serial.println("Trolley moved to the captured piece");   
      ...
      if (stop_execution) return;
      Serial.println("Trolley moved to the departure piece");   
    }
    ...
    if (i == 0) { // removes the captured piece to the corner
      ...
      Serial.print("Stacking Offset: ");
      Serial.println(stacking_offset);
      Serial.print("Additional Displacement: ");
      Serial.println(additional_displacement);
      Serial.print("Return Movement: ");
  ...
  Serial.println("Moving piece to destination...");
